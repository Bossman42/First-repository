...
const int transmit = A4;
const int transmitled = A5;


const int  delayTime = 5;  // DIgit refresh time

const int programed = 13;
const int transmitTrigger = 1;
...
int programedState = 0;
int transmitState = 0;
int transmitledState = 0;
...
void setup() {                
  ...
  pinMode (transmit,INPUT_PULLUP);
  pinMode (programed,INPUT_PULLUP);// when all leds are lit correctly
  pinMode (transmitTrigger,OUTPUT);
  
  digitalWrite (transmitTrigger,HIGH);
...
void loop() {
 //1st Button 
  ...
  programedState = digitalRead(programed);
  transmitState = digitalRead (transmit);
  transmitledState= digitalRead (transmitled);
...
if (buttonPushCounter == freqPos1 && buttonPushCounter2 == freqPos2 && buttonPushCounter3 == freqPos3 && buttonPushCounter4 == freqPos4 && programedState == LOW) {
  digitalWrite (transmitled,LOW);}
  else digitalWrite (transmitled,HIGH);

if (transmitState == LOW && transmitledState == LOW)
{  digitalWrite (transmitTrigger, LOW);}
